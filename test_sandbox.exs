...
sandbox_supervisors = Enum.filter(all_supervisors, fn supervisor ->
...
case OTPSupervisor.Core.Control.create_sandbox(:otp_sandbox) do
  {:ok, sandbox_info} ->
    ...
    new_sandbox_supervisors = Enum.filter(new_supervisors, fn supervisor ->
      name_str = to_string(supervisor.name)
      String.contains?(name_str, "sandbox") or 
      String.contains?(name_str, "Sandbox") or
      String.contains?(name_str, "OtpSandbox")
    ...
    IO.puts("Found #{length(new_sandbox_supervisors)} sandbox-related supervisors after creation:")
