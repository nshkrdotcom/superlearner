...
# Add the playwriter library to the path
Code.append_path("../playwriter/playwriter/_build/dev/lib/playwriter/ebin")
Code.append_path("../playwriter/playwriter/_build/dev/lib/playwright/ebin")

# Also try to load from the lib directory directly
Code.require_file("../playwriter/playwriter/lib/playwriter.ex")

defmodule RealHeightChecker do
  @moduledoc """
  Use Playwriter to check actual rendered header heights from DevTools
  """

  def check_real_heights do
    IO.puts("🔍 Checking ACTUAL rendered header heights using Playwriter DevTools...")
    ...
    # Use Playwriter to run browser automation
    browser_opts = %{headless: false}

    Enum.each(pages, fn page_info ->
      case measure_header_with_playwriter(page_info, browser_opts) do
        {:ok, measurements} ->
  ...
  defp measure_header_with_playwriter(page_info, browser_opts) do
    Playwriter.with_browser(browser_opts, fn page ->
      # Navigate to the page
      Playwright.Page.goto(page, page_info.url)
      
      # Wait for the page to fully load
      :timer.sleep(3000)
      
      # Execute JavaScript to get actual DevTools measurements
      measurements = Playwright.Page.evaluate(page, """
        (() => {
