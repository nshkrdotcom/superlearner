...
defmodule ArsenalTest do
  def create_sandbox(id, module) do
    ...
    with {:ok, validated} <-
           OTPSupervisor.Core.Arsenal.Operations.CreateSandbox.validate_params(params),
         {:ok, sandbox_info} <-
           OTPSupervisor.Core.Arsenal.Operations.CreateSandbox.execute(validated) do
      formatted =
  ...
  def list_sandboxes() do
    ...
    with {:ok, validated} <-
           OTPSupervisor.Core.Arsenal.Operations.ListSandboxes.validate_params(params),
         {:ok, {sandboxes, meta}} <-
           OTPSupervisor.Core.Arsenal.Operations.ListSandboxes.execute(validated) do
      formatted =
        OTPSupervisor.Core.Arsenal.Operations.ListSandboxes.format_response({sandboxes, meta})
  ...
  def get_sandbox_info(id) do
    ...
    with {:ok, validated} <-
           OTPSupervisor.Core.Arsenal.Operations.GetSandboxInfo.validate_params(params),
         {:ok, sandbox_info} <-
           OTPSupervisor.Core.Arsenal.Operations.GetSandboxInfo.execute(validated) do
      formatted =
        OTPSupervisor.Core.Arsenal.Operations.GetSandboxInfo.format_response(sandbox_info)
  ...
  def destroy_sandbox(id) do
    ...
    with {:ok, validated} <-
           OTPSupervisor.Core.Arsenal.Operations.DestroySandbox.validate_params(params),
