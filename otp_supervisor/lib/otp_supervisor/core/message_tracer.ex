defmodule OTPSupervisor.Core.MessageTracer do
  ...
  def handle_info({:trace_ts, pid, :send, message, to, timestamp}, state) when pid == state.traced_pid do
    new_message = %{
  ...
  def handle_info({:trace_ts, pid, :receive, message, timestamp}, state) when pid == state.traced_pid do
    new_message = %{
      timestamp: timestamp_to_milliseconds(timestamp),
      direction: :incoming,
      content: message
