defmodule OTPSupervisor.Distributed.ArsenalDistributedTest do
  ...
  describe "ClusterHealth operation" do
    test "returns healthy status for simulated cluster" do
      ...
      assert data.nodes_total == 3  # Default simulation has 2 simulated + 1 real node
    ...
    test "detects node failures in simulation" do
      ...
      assert data.nodes_total == 3  # 1 real + 2 simulated nodes
      assert data.nodes_healthy == 2  # 1 real + 1 healthy simulated
      assert data.nodes_unhealthy == 1  # 1 failed simulated
    ...
    test "works with real multi-node cluster" do
      ...
      assert_cluster_size(2)  # Primary + 1 test node
      
      data = time_execution("ClusterHealth operation", fn ->
  ...
  describe "ClusterTopology operation" do
    test "returns complete topology information" do
      data = assert_arsenal_operation_succeeds(ClusterTopology, %{
        "include_health" => true
      })
      
      assert is_list(data.nodes)
      assert length(data.nodes) == 3  # 1 real + 2 simulated nodes
      assert data.total_nodes == 3
      assert data.mode == :single_node  # Simulation doesn't change ToolManager mode
  ...
  describe "NodeInfo operation" do
    test "returns detailed information for simulated node" do
      ...
      data = assert_arsenal_operation_succeeds(NodeInfo, %{
    ...
    test "fails for non-existent node" do
      assert_arsenal_operation_fails(NodeInfo, %{
  ...
  describe "ProcessList operation" do
    test "lists processes across simulated cluster" do
      data = assert_arsenal_operation_succeeds(ProcessList, %{
        "limit" => 50,
        "include_details" => true
      })
      ...
      assert length(data.nodes_queried) == 3  # 1 real + 2 simulated nodes
  ...
  describe "error handling" do
    test "operations handle simulation failures gracefully" do
      ...
      node_data = assert_arsenal_operation_succeeds(NodeInfo, %{
        "node" => Atom.to_string(first_node)
      })
    ...
    test "operations handle network partitions" do
      ...
      {:ok, partitioned_nodes} = OTPSupervisor.Distributed.SingleNodeSimulator.simulate_network_partition([node1, node2])
      
      health_data = assert_arsenal_operation_succeeds(ClusterHealth, %{})
      
      # Should detect partition in some form
      partitioned_count = Enum.count(health_data.node_statuses, fn {_node, status} ->
  ...
  describe "performance" do
    ...
    test "operations scale with real cluster size" do
      ...
      assert_cluster_size(3)  # Primary + 2 test nodes
      
      {time, data} = time_execution("ClusterHealth with 3 nodes", fn ->
