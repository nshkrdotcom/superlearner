defmodule OTPSupervisor.DistributedTestCase do
  ...
  defp setup_simulation_context(context, tags) do
    ...
    %{
      ...
      | simulation_enabled: true,
        cleanup_functions: [
          (&OTPSupervisor.Distributed.SingleNodeSimulator.disable_simulation/0)
  ...
  defp setup_real_nodes_context(context, tags) do
    ...
    # +1 for primary
    :ok = ClusterTestHelper.wait_for_cluster(node_count + 1)

    cleanup_fn = fn ->
      Enum.each(nodes, &ClusterTestHelper.stop_test_node/1)
    end

    %{context | test_nodes: nodes, cleanup_functions: [cleanup_fn | context.cleanup_functions]}
  end
