defmodule Mix.Tasks.Test.Config do
  ...
  defp display_port_config(config) do
    ...
    Mix.shell().info("  Distribution Port Range: #{elem(port_ranges.distribution, 0)}-#{elem(port_ranges.distribution, 1)}")
  ...
  defp validate_configuration(config, opts) do
    ...
    case ConfigValidator.validate_with_warnings(config) do
      ...
      {:error, report} ->
        ...
        Mix.shell().error("Found #{report.error_count} errors and #{report.warning_count} warnings")
