defmodule OTPSupervisor.TestCluster.NodeProvisioner do
  ...
  defp start_node_with_retry(node_config, attempt \\ 1) do
    ...
    case start_node(node_config) do
      ...
      {:error, reason} ->
        Logger.error("Node startup failed after #{@max_startup_retries} attempts: #{inspect(reason)}")
  ...
  defp start_with_peer(node_config) do
    ...
    peer_args = %{
      ...
      host: ~c"127.0.0.1",  # Use 127.0.0.1 explicitly
      args: [
        ~c"-setcookie", Atom.to_charlist(node_config.cookie),
        ~c"-kernel", ~c"inet_dist_listen_min", Integer.to_charlist(node_config.dist_port),
        ~c"-kernel", ~c"inet_dist_listen_max", Integer.to_charlist(node_config.dist_port)
      ],
      env: [
        {~c"MIX_ENV", ~c"test"},
