defmodule OTPSupervisor.Core.IsolatedCompiler do
  ...
  def compile_sandbox(sandbox_path, opts \\ []) do
    ...
    with :ok <- validate_sandbox_path(sandbox_path),
      ...
      Logger.debug("Sandbox compiled successfully in #{compilation_time}ms", 
        sandbox_path: sandbox_path, artifacts: length(artifacts.beam_files))
      
      {:ok, compile_info}
    else
      {:error, reason} ->
        Logger.warning("Sandbox compilation failed", 
          sandbox_path: sandbox_path, reason: inspect(reason))
        {:error, reason}
    end
  end
  ...
  defp compile_in_isolation(sandbox_path, temp_dir, timeout, opts) do
    ...
    compiler_pid = spawn(fn ->
      try do
        ...
        # Compile with mix
        {result, exit_code} = System.cmd("mix", ["compile", "--force"], 
  ...
  defp collect_compilation_artifacts(sandbox_path, temp_dir) do
    ...
    # Locate .app file
    app_file_pattern = Path.join([temp_dir, "lib", to_string(app_name), "ebin", "#{app_name}.app"])
    app_file = case Path.wildcard(app_file_pattern) do
      [app_file] -> app_file
      [] -> nil
  ...
  defp validate_beam_file(beam_file) do
    try do
      case :beam_lib.info(String.to_charlist(beam_file)) do
        ...
        {:error, reason} ->
          {:error, "BEAM lib error: #{inspect(reason)}"}
        other ->
          # Some beam_lib.info returns just a list of chunks instead of {:ok, info}
          if is_list(other) do
            :ok
          else
            {:error, "Unexpected beam_lib result: #{inspect(other)}"}
          end
      end
    rescue
