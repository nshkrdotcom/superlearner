  ...
  def execute(%{"sandbox_id" => sandbox_id, "module" => module_name}) do
    case OTPSupervisor.Core.SandboxManager.get_sandbox_info(sandbox_id) do
      {:ok, sandbox_info} ->
        ...
        # Get current version
        current_version = case OTPSupervisor.Core.ModuleVersionManager.get_current_version(sandbox_id, target_module) do
          {:ok, version} -> version
          {:error, :not_found} -> 0
        end
                ...
                case File.read(beam_file) do
                  {:ok, beam_data} ->
                    ...
                    case OTPSupervisor.Core.ModuleVersionManager.hot_swap_module(
                      sandbox_id, target_module, beam_data
                    ) do
                      {:ok, :hot_swapped} ->
                        {:ok, new_version} = OTPSupervisor.Core.ModuleVersionManager.get_current_version(sandbox_id, target_module)
