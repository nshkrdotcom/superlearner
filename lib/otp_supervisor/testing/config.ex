defmodule OTPSupervisor.Testing.Config do
  ...
  @default_config %{
    ...
    # Port management
    http_port_base: 4200,  # Different from dev (4000) to avoid conflicts
    dist_port_base: 9200,  # Different from dev (9100) to avoid conflicts
    port_range_size: 100,
    
    # Environment detection
    ci_mode: nil,  # Auto-detect if nil
    ci_cluster_size: 2,
  ...
  def validate_config(config) do
    ...
    case errors do
      [] -> 
        config
      _ -> 
        error_message = "Invalid distributed testing configuration:\n" <> 
                       Enum.join(errors, "\n")
  ...
  defp validate_cluster_sizes(config, errors) do
    errors = if config.default_cluster_size < 1 do
    ...
    errors = if config.min_cluster_size < 1 do
  ...
  defp validate_ports(config, errors) do
    errors = if config.http_port_base < 1024 or config.http_port_base > 65535 do
      ["http_port_base must be between 1024 and 65535" | errors]
    else
      errors
    end
    
    errors = if config.dist_port_base < 1024 or config.dist_port_base > 65535 do
      ["dist_port_base must be between 1024 and 65535" | errors]
    else
      errors
    end
    
    errors = if config.port_range_size < 10 or config.port_range_size > 1000 do
  ...
  defp adjust_for_environment(config) do
    case config.detected_environment do
      :ci ->
        ...
        |> Map.put(:verbose_cluster_logs, true)  # More logging in CI
