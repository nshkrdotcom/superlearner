defmodule OTPSupervisor.Testing.ConfigValidator do
  ...
  defp validate_basic_config(config) do
    ...
    errors =
      if not is_boolean(config.auto_cluster) do
        [
          %{
            type: :invalid_type,
            field: :auto_cluster,
    ...
    errors =
      if not is_integer(config.default_cluster_size) or config.default_cluster_size < 1 do
        [
          %{
            type: :invalid_value,
  ...
  defp validate_environment_config(config) do
    ...
    errors =
      if not is_boolean(config.ci_mode) do
        [
          %{
            type: :invalid_type,
            field: :ci_mode,
    ...
    errors =
      if not is_integer(config.ci_cluster_size) or config.ci_cluster_size < 1 do
        [
          %{
            type: :invalid_value,
  ...
  defp validate_port_config(config) do
    ...
    errors =
      if not is_integer(config.http_port_base) or
    ...
    errors =
      if not is_integer(config.dist_port_base) or
    ...
    errors =
      if not is_integer(config.port_range_size) or
  ...
  defp validate_cluster_config(config) do
    ...
    errors =
      if config.ci_cluster_size > config.max_cluster_size do
        [
          %{
            type: :inconsistent_values,
  ...
  defp validate_resource_constraints(config) do
    ...
    # Rough estimate
    total_estimated_processes = config.max_cluster_size * 10
  ...
  defp validate_port_conflicts(config) do
    ...
    conflicts =
      Enum.filter(common_dev_ports, fn port ->
        in_http_range = port >= elem(port_ranges.http, 0) and port <= elem(port_ranges.http, 1)

        in_dist_range =
          port >= elem(port_ranges.distribution, 0) and port <= elem(port_ranges.distribution, 1)
  ...
  defp check_performance_warnings(config) do
    ...
    warnings =
      if config.cluster_startup_timeout > 60_000 do
        [
          %{
  ...
  defp check_configuration_warnings(config) do
    ...
    warnings =
      if not config.cleanup_on_exit do
        [
          %{
