defmodule OTPSupervisor.Testing.DistributedTestCase do
  ...
  """
  def with_cluster_size(required_size, test_fun)
      when is_integer(required_size) and required_size > 0 do
    ...
    if length(current_nodes) >= required_size do
      ...
      exit(
        {:skip, "Insufficient cluster size: need #{required_size}, have #{length(current_nodes)}"}
      )
    end
  end
    ...
    try do
      ...
      :exit, {:noproc, _} ->
        {:error,
         %{
           problem:
             "AutoClusterManager is not running - distributed test infrastructure not available",
           solutions: [
