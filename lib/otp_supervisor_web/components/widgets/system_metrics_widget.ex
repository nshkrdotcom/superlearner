defmodule OtpSupervisorWeb.Components.Widgets.SystemMetricsWidget do
          ...
          <%= if @show_charts do %>
            <div class="h-8 flex items-end space-x-px">
              <%= for value <- (@metrics.cpu[:history] || []) do %>
                <div 
                  class="flex-1 bg-green-500/30 rounded-t"
                  style={"height: #{value}%"}
                ></div>
              <% end %>
            </div>
  ...
  defp memory_progress_color(used, total) when is_number(used) and is_number(total) and total > 0 do
    percentage = (used / total) * 100
    cond do
  ...
  defp has_alerts?(assigns) do
    ...
    cond do
      Map.get(thresholds, :cpu_threshold) && metrics.cpu.usage >= thresholds.cpu_threshold -> true
      Map.get(thresholds, :memory_threshold) && 
        (metrics.memory.used / metrics.memory.total * 100) >= thresholds.memory_threshold -> true
      Map.get(thresholds, :load_threshold) && metrics.cpu.load_average >= thresholds.load_threshold -> true
      true -> false
    end
  end
  ...
  defp get_active_alerts(assigns) do
    ...
    alerts = if Map.get(thresholds, :cpu_threshold) && metrics.cpu.usage >= thresholds.cpu_threshold do
      [%{message: "High CPU usage: #{metrics.cpu.usage}%", severity: :critical} | alerts]
    else
      alerts
    end

    alerts = if Map.get(thresholds, :memory_threshold) && 
    ...
    alerts = if Map.get(thresholds, :load_threshold) && metrics.cpu.load_average >= thresholds.load_threshold do
      [%{message: "High load average: #{metrics.cpu.load_average}", severity: :warning} | alerts]
    else
      alerts
    end
