defmodule OtpSupervisorWeb.Components.Widgets.ExecutionPanelWidget do
        ...
        <%= if @executions == [] do %>
          <div class="flex-1 flex items-center justify-center">
            <div class="text-center">
              ...
              <div class="text-green-400/30 text-xs font-mono mt-1">Execute an operation to see results here</div>
            </div>
          </div>
        <% else %>
          <div class="flex-1 overflow-auto">
            <div class="space-y-2 p-4">
              <%= for {execution, index} <- Enum.with_index(@executions) do %>
                <div class={[
  ...
  defp calculate_avg_duration(executions) do
    completed = Enum.filter(executions, &(&1.status == :completed and &1.started_at and &1.completed_at))
    
    if length(completed) == 0 do
      "N/A"
    else
      total_duration = Enum.reduce(completed, 0, fn execution, acc ->
