...
case OTPSupervisor.Core.IsolatedCompiler.compile_sandbox(
  ...
  {:ok, compile_info} ->
    ...
    supervisor_beam =
      Enum.find(compile_info.beam_files, fn beam_file ->
        ...
        {:ok, beam_data} ->
          ...
          case OTPSupervisor.Core.ModuleVersionManager.hot_swap_module(
            ...
            {:ok, :hot_swapped} ->
              ...
              IO.puts(
                "Check manually: curl -s 'http://localhost:4000/api/v1/sandboxes/web-test-1?include_children=true' | jq '.data.children'"
              )
